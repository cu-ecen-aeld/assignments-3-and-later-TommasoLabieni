...
#define OUT_FILENAME "/dev/aesdchar"
#else
...
static void sig_handler_f(int sig_num) {
...
void execute_server() {
  ...
  for (;;) {
    cfd = accept(sock_fd, (struct sockaddr *)&peer_addr, &peer_addr_size);
    if (cfd == -1) {
    ...
    if (t_data == NULL) {
      syslog(LOG_ERR,
             "Error while allocating space for thread data. Errno message is: "
             "\n%s\n",
             strerror(errno));
      closelog();
      close(sock_fd);
    ...
    if (t_data->tid == NULL) {
      syslog(LOG_ERR,
             "Error while allocating space for thread ID. Errno message is: "
             "\n%s\n",
             strerror(errno));
      closelog();
      close(sock_fd);
    ...
    if (pthread_create(t_data->tid, NULL, handle_connection, (void *)t_data) !=
        0) {
...
int main(int argc, char *argv[]) {
  ...
  if (daemon_mode) {
    ...
    if (sock_pid == -1) {
      syslog(LOG_ERR,
             "Error while creating child process. Errno message is: \n%s\n",
             strerror(errno));
      closelog();
      exit(-1);
    }
